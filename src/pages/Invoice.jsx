...
const Invoice = () => {
        ...
        axios.get('https://jsonplaceholder.typicode.com/posts')
            .then(response => {
                const invoiceData = response.data.map(item => ({
                    ...
                    paid: parseFloat((Math.random() * 500).toFixed(2)),  // Random paid amount
    ...
    const deleteInvoice = (invoiceId) => {
        // Make a request to your backend API to delete the invoice
    ...
    const handleDelete = (invoiceId) => {
        confirmAlert({
            customUI: ({ onClose }) => (
                <div
                    className="fixed top-0 right-0 bottom-0 left-0 flex justify-center items-center bg-gray-500 bg-opacity-75"
                >
                    <div className="bg-white rounded-lg shadow-md p-4 w-64">
                        ...
                        <button
                            className="bg-primary-light hover:bg-primary text-white font-bold py-2 px-4 rounded"
                        ...
                        {currentItems.map(invoice => (
                            <tr key={invoice.id} className='border-b border-secondary-light hover:bg-gray-100'>
                                ...
                                <td className='text-center p-2'>
                                    <Link to={`/invoices/${invoice.id}`} className='text-sm px-2 mx-1 rounded text-white bg-secondary '>View</Link>
                                    <Link to={`/invoices/edit/${invoice.id}`} className='text-sm px-2 mx-1 rounded text-white bg-secondary '>Edit</Link>
                                    <button onClick={() => handleDelete(invoice.id)} className='text-sm px-2 mx-1 rounded text-white bg-secondary '>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
